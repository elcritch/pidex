defmodule PidexTest do
  ...
  test "test pid ser" do
    alias Pidex.PdxServer

    sample_time = 0.02
    settings = %Pidex{kP: 1.2, kI: 1.0, kD: 0.001,
                          min_point: -20.0, max_point: 20.0}
    {:ok, pid} = PdxServer.start_link(settings: settings)
    ...
    results =
      for i <- 1..50, into: [] do
          ...
          IO.puts "update PdxServer "
          output = pid |> PdxServer.update(feedback, sample_time*i)
          IO.puts "updated PdxServer "

          IO.puts "pid out: #{inspect output} "
